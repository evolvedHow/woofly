...
export default function BottomNav({ showAi }) {
  ...
  return (
    <nav
      className={`border-t border-slate-100 dark:border-slate-800 bg-white/95 dark:bg-slate-900/95 backdrop-blur pb-[env(safe-area-inset-bottom)] ${
        framed
...
export function AiChatLink({ className = '' }) {
  ...
  return (
    <NavLink
      to="/ai"
      className={`right-4 bottom-24 z-40 flex items-center gap-1.5 rounded-full bg-woof-ink dark:bg-white text-white dark:text-woof-ink px-4 py-2.5 text-xs font-extrabold shadow-card active:scale-95 transition ${
